...
def wkt_to_shp(wkt_list, shp_file):
    """Write wkt linestring list to shapefile using geopandas."""
    import geopandas as gpd
    import shapely.wkt
    shapes = [shapely.wkt.loads(line) for line in wkt_list]
    gdf = gpd.GeoDataFrame({'id': list(range(len(shapes))), 'geometry': shapes})
    gdf.to_file(shp_file)


###############################################################################
def get_node_geo_coords(G, im_file, verbose=False):
    """Convert node pixel coordinates to geographic coordinates."""
    ...
    for i, (n, attr_dict) in enumerate(G.nodes(data=True)):
        ...
        # Use pyproj to get UTM coordinates
        utm_zone = int((lon + 180) / 6) + 1
        utm_crs_str = f"+proj=utm +zone={utm_zone} +datum=WGS84 +units=m"
        transformer = Transformer.from_crs('EPSG:4326', utm_crs_str, always_xy=True)
        utm_east, utm_north = transformer.transform(lon, lat)
        utm_letter = ''  # deprecated, set empty
        attr_dict['lon'] = lon
        attr_dict['lat'] = lat
...
def get_edge_geo_coords(G, im_file, remove_pix_geom=True,
                        verbose=False):
    """Convert edge pixel coordinates to geographic coordinates."""
...
if __name__ == "__main__":
    ...
    for i, image_id in enumerate(image_ids):
        ...
        with open(out_file, 'wb') as f:
            pickle.dump(G, f, protocol=pickle_protocol)
