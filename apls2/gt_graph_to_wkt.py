...
def gt_geojson_to_wkt(geojson_path, im_path,
                      ...
                      refine=True,
                      super_verbose=False):
    """Create wkt list of pixel coords in ground truth graph.

    For use in SpaceNet TopCoder competition.
...
def gt_geojson_dir_to_wkt(geojson_dir, im_dir, output_csv_path,
                     ...
                     verbose=False,
                     super_verbose=False):
    """Convert a directory of geojson road files to a WKT CSV.

    Iterates through all geojson files in geojson_dir, matches each to the
    corresponding image chip in im_dir, builds the ground truth graph, and
    writes all WKT linestrings to output_csv_path.
    """
