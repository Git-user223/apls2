...
def speed_func(geojson_row, label_type='sn5', verbose=True):
    ...
    if label_type == 'osm':
        ...
        skip_set = (
            'stopline', 'footway', 'bridleway', 'step', 'steps',
        ...
        if road_type_str in skip_set:
            if verbose:
                print(f"road_type {road_type_str} in skip_set")
            return -1, -1
        else:
            if verbose:
    ...
    # SpaceNet 5 labels
        ...
        # e.g., { "type": "Feature", "properties": { "OBJECTID": "43",
        #   "bridge": null, "highway": "unclassified", "osm_id": 683685519.0,
    ...
    #   Use Oregon:
    #       State    Motorway  Trunk   Primary  Secondary  Tertiary
    #       Oregon   55 mph    55 mph  55 mph   35 mph     30 mph
    #              Unclassified  Residential  Living street  Service
    #                25 mph       25 mph         15 mph
...
def speed_to_bins_bg(speed_mph, bin_size_mph=10.0, channel_value_mult=1.):
    '''Convert speed estimate to appropriate channel bin.
...
def update_feature_name(geojson_row, name_bad, name_good):
    '''Rename a property key in a geojson feature row in place.'''
...
def add_speed_to_geojson(geojson_path_in, geojson_path_out,
    ...
    with open(geojson_path_in, 'r+') as f:
        ...
        for i, geojson_row in enumerate(geojson_data['features']):
            ...
            speed_mph, speed_mps = speed_func(
                geojson_row, label_type=label_type, verbose=verbose
            )
            if verbose:
    ...
    if len(bad_row_idxs) > 0:
        geojson_data['features'] = [
            e + '\n' for i, e in enumerate(geojson_data['features'])
            if i not in bad_row_idxs
        ]
...
def update_geojson_dir_speed(geojson_dir_in, geojson_dir_out,
    ...
    '''Update geojson data to add inferred speed information for an entire
    ...
    json_files = np.sort([
        j for j in os.listdir(geojson_dir_in) if j.endswith('.geojson')
    ])
    if super_verbose:
    ...
    for i, json_file in enumerate(json_files):
        ...
        geojson_path_out = os.path.join(
            geojson_dir_out, root + suffix + '.' + ext
        )
        sarr, sset = add_speed_to_geojson(
            geojson_path_in, geojson_path_out,
...
def _read_image_shape(image_path):
    '''Read image height and width using rasterio without loading all data.
...
def _read_image_array(image_path):
    '''Read an image into a numpy array using rasterio.

    Returns array with shape (height, width, bands) for multi-band images
    or (height, width) for single-band images.
    '''

    with rasterio.open(image_path) as src:
        data = src.read()  # shape: (bands, height, width)
    if data.shape[0] == 1:
        return data[0]  # (height, width)
    return np.moveaxis(data, 0, -1)  # (height, width, bands)


###############################################################################
def _write_image_array(image_path, array, reference_path=None):
    '''Write a numpy array to disk as a GeoTIFF using rasterio.

    Copies georeferencing from reference_path if provided.
    Supports single-band (height, width) or multi-band (height, width, bands).
    '''

    if array.ndim == 2:
        height, width = array.shape
        count = 1
        write_data = array[np.newaxis, ...]  # (1, height, width)
    else:
        height, width, count = array.shape
        write_data = np.moveaxis(array, -1, 0)  # (bands, height, width)
    ...
    if reference_path is not None:
        ...
        profile.update(count=count, dtype=str(dtype), height=height, width=width)
    else:
        profile = {
            'driver': 'GTiff',
            'count': count,
...
def create_speed_gdf(image_path, geojson_path, mask_path_out_gray,
    ...
    try:
        ...
        _write_image_array(mask_path_out_gray, mask_gray,
    ...
    gdf_utm_buffer['geometry'] = gdf_utm_buffer.buffer(
        bufferDistanceMeters, bufferRoundness
    )
    ...
    # largely black)
    im_array = _read_image_array(image_path)
    if im_array.ndim == 3:
        im_gray = np.sum(im_array, axis=2)
    else:
        im_gray = im_array
    ...
    if zero_frac >= zero_frac_thresh:
        ...
        print("create_speed_gdf(): checking to ensure masks are null where "
              "image is null")
        # read back the mask, zero out where image is zero, and overwrite
        mask_gray = _read_image_array(mask_path_out_gray)
