...
def plot_graph_on_im(G_, im_test_file, figsize=(8, 8), show_endnodes=False,
    ...
    with rasterio.open(im_test_file) as src:
        img_arr = src.read()
    # rasterio reads as (bands, height, width); matplotlib wants (height, width, bands)
    if img_arr.shape[0] == 1:
        img_mpl = img_arr[0]  # grayscale
    elif img_arr.shape[0] >= 3:
        img_mpl = np.moveaxis(img_arr[:3], 0, -1)  # take first 3 bands as RGB
    else:
        img_mpl = np.moveaxis(img_arr, 0, -1)
    h, w = img_mpl.shape[:2]
