...
Modernized for current geopandas / pyproj API (no legacy CRS dicts, no utm
...
def project_gdf(gdf, to_crs=None, to_latlong=False):
    ...
    Modernized from osmnx v0.9 projection.py to use current geopandas API
    (no legacy CRS dicts, uses estimate_utm_crs()).
    ...
    to_latlong : bool
        if True, projects to EPSG:4326 instead of to UTM
    ...
    if to_crs is not None:
        ...
        if to_latlong:
            ...
            # if already projected (e.g. UTM), return as-is
            if gdf.crs is not None and gdf.crs.is_projected:
                return gdf
            # estimate UTM CRS from data centroid
...
def project_graph(G, to_crs=None):
    """
    Project a graph from lat-long to the UTM zone appropriate for its
    geographic location.
    ...
    gdf_nodes = gdf_nodes.set_crs(G_proj.graph['crs'])
    ...
    for u, v, key, data in G_proj.edges(keys=True, data=True):
        if 'geometry' in data:
            edges_with_geom.append(
                {'u': u, 'v': v, 'key': key, 'geometry': data['geometry']}
            )
    ...
    if len(edges_with_geom) > 0:
        ...
        gdf_edges = gdf_edges.set_crs(G_proj.graph['crs'])
    ...
    for u, v, key, attributes in edges:
        ...
        # attributes dict contains key, so we don't need to explicitly pass it
...
def graph_to_gdfs(G, nodes=True, edges=True, node_geometry=True,
    ...
    if nodes:
        ...
        gdf_nodes = gdf_nodes.set_crs(G.graph['crs'])
    ...
    if edges:
        ...
        gdf_edges = gdf_edges.set_crs(G.graph['crs'])
...
def plot_graph(G, bbox=None, fig_height=6, fig_width=None, margin=0.02,
    ...
    # get north, south, east, west values either from bbox parameter or from
    # the spatial extent of the edges' geometries
    if bbox is None:
    ...
    for u, v, data in G.edges(keys=False, data=True):
        if 'geometry' in data and use_geom:
            # if it has a geometry attribute (a list of line segments), add
            # them to the list of lines to plot
    ...
    if equal_aspect:
        ...
        # stretch the plot
        if G.graph.get('crs', '') == default_crs:
            coslat = np.cos(
                (min(node_Ys) + max(node_Ys)) / 2. / 180. * np.pi
            )
    ...
    fig, ax = save_and_show(
        fig, ax, save, show, close, file_format, dpi, axis_off,
        filename=filename
    )
    return fig, ax
...
def node_list_to_coordinate_lines(G, node_list, use_geom=True):
    """
    Given a list of nodes, return a list of lines that together follow the
    path defined by the list of nodes.
...
def plot_graph_route(G, route, bbox=None, fig_height=6, fig_width=None,
                     ...
                     save=False, close=True, file_format='png',
                     filename='temp', dpi=300, annotate=False,
                     node_color='#999999', node_size=15, node_alpha=1,
                     node_edgecolor='none', node_zorder=1,
                     edge_color='#999999', edge_linewidth=1, edge_alpha=1,
                     use_geom=True, origin_point=None, destination_point=None,
                     route_color='r', route_linewidth=4, route_alpha=0.5,
                     orig_dest_node_alpha=0.5, orig_dest_node_size=100,
    ...
    node_edgecolor : string
        the color of the node's marker's border
    node_zorder : int
        zorder to plot nodes, edges are always 2, so make node_zorder 1 to
        plot nodes beneath them or 3 to plot nodes atop them
    edge_color : string
        the color of the edges' lines
    edge_linewidth : float
        the width of the edges' lines
    ...
    # the origin and destination nodes are the first and last nodes in the
    # route
    origin_node = route[0]
    destination_node = route[-1]

    if origin_point is None or destination_point is None:
        # if caller didn't pass points, use the first and last node in route
    ...
    ax.scatter(
        origin_destination_lons, origin_destination_lats,
        s=orig_dest_node_size, c=orig_dest_node_color,
    ...
    # add the lines to the axis as a linecollection
    lc = LineCollection(
        lines, colors=route_color, linewidths=route_linewidth,
    ...
    # save and show the figure as specified
    fig, ax = save_and_show(
        fig, ax, save, show, close, filename, file_format, dpi, axis_off
    )
    return fig, ax
...
def save_and_show(fig, ax, save, show, close, file_format, dpi, axis_off,
    ...
    if save:
        ...
        if file_format == 'svg':
            ...
            if len(filename) > 0:
                fig.savefig(
                    path_filename, bbox_inches=0, format=file_format,
                    facecolor=fig.get_facecolor(), transparent=True
                )
        else:
            if axis_off:
                # if axis is turned off, constrain the saved figure's extent
                # to the interior of the axis
            ...
            if len(filename) > 0:
                fig.savefig(
                    path_filename, dpi=dpi, bbox_inches=extent,
...
def is_endpoint(G, node, strict=True):
    ...
    # endpoint
        ...
        # if there is more than 1 OSM ID in the list of edge OSM IDs then it
        # is an endpoint, if not, it isn't
...
def get_paths_to_simplify(G, strict=True):
    ...
    nodes, to the second endpoint. If your street network is in a rural area
    with many interstitial nodes between true edge endpoints, you may want to
    increase your system's recursion limit to avoid recursion errors.
    ...
    print('Identified {:,} edge endpoints in {:,.2f} seconds'.format(
        len(endpoints), time.time() - start_time
    ))
        ...
        for successor in G.successors(node):
            if successor not in endpoints:
                ...
                try:
                    ...
                    print(
                        'Recursion error: exceeded max depth, moving on to '
                        'next endpoint successor'
                    )
                    # recursion errors occur if some connected component is a
                    # self-contained ring in which all nodes are not end
                    # points. could also occur in extremely long street
                    # segments (eg, in rural areas) with too many nodes
                    # between true endpoints. handle it by just ignoring that
                    # component and letting its topology remain intact (this
                    # should be a rare occurrence). RuntimeError is what
                    # Python <3.5 will throw, Py3.5+ throws RecursionError
                    # but it is a subtype of RuntimeError so it still gets
                    # handled

    print('Constructed all paths to simplify in {:,.2f} seconds'.format(
...
def simplify_graph(G, strict=True):
    """
    Simplify a graph's topology by removing all nodes that are not
    ...
    for path in paths:
        ...
        for u, v in zip(path[:-1], path[1:]):
            ...
            if not G.number_of_edges(u, v) == 1:
                print('Multiple edges between "{}" and "{}" found when '
                      'simplifying'.format(u, v))

            # the only element in this list as long as above check is True
            # (MultiGraphs use keys (the 0 here), indexed with ints from 0
            # and up)
            edge = G.edges[u, v, 0]
            for key in edge:
                if key in edge_attributes:
                    ...
                    # if this key doesn't already exist, set the value to a
                    # list containing the one value
    ...
    msg = ('Simplified graph (from {:,} to {:,} nodes and from {:,} to {:,} '
           'edges) in {:,.2f} seconds')
    print(msg.format(
        initial_node_count, len(list(G.nodes())),
        initial_edge_count, len(list(G.edges())),
...
def clean_intersections(G, tolerance=15, dead_ends=False):
    ...
    single intersection in the real world. This function cleans them up by
    buffering their points to an arbitrary distance, merging overlapping
    buffers, and taking their centroid. For best results, the tolerance
    argument should be adjusted to approximately match street design standards
...
def great_circle_vec(lat1, lng1, lat2, lng2, earth_radius=6371009):
    ...
    earth_radius : numeric
        radius of earth in units in which distance will be returned (default
        is meters)
...
def get_largest_component(G, strongly=False):
    ...
    if strongly:
        # if the graph is not connected retain only the largest strongly
        # connected component
        if not nx.is_strongly_connected(G):

            # get all the strongly connected components in graph then identify
            # the largest
            sccs = nx.strongly_connected_components(G)
            largest_scc = max(sccs, key=len)
            G = induce_subgraph(G, largest_scc)

            msg = ('Graph was not connected, retained only the largest '
                   'strongly connected component ({:,} of {:,} total nodes) '
                   'in {:.2f} seconds')
            print(msg.format(
                len(list(G.nodes())), original_len, time.time() - start_time
            ))
    else:
        # if the graph is not connected retain only the largest weakly
        # connected component
        if not nx.is_weakly_connected(G):

            # get all the weakly connected components in graph then identify
            # the largest
            ...
            msg = ('Graph was not connected, retained only the largest weakly '
                   'connected component ({:,} of {:,} total nodes) in '
                   '{:.2f} seconds')
            print(msg.format(
                len(list(G.nodes())), original_len, time.time() - start_time
            ))
...
def add_edge_lengths(G):
    """
    Add length (meters) attribute to each edge by great circle distance
    ...
    print('Added edge lengths to graph in {:,.2f} seconds'.format(
        time.time() - start_time
    ))
    return G
